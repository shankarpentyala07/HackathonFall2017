...
app.listen(port, function() {
  console.log('RESUME PROJECT RUNNING ON: http://localhost:' + port)
});
...
app.post('/upload', function(req, res){       
  ...
  var resumeContent = pdfParser.getRawTextContent();
      var c = new Client();
      c.on('ready', function() {
        c.put(file.path, 'htd'+timestamp+"-"+file.originalFilename, function(err) {
          if (err) throw err;
          c.end();
        });
      });
      ...
      var resumeurl = 'localhost'+timestamp+"-"+file.originalFilename;
      var newResume = {
        fullname: fullname,
        email: email,
...
app.post('/upload2', function(req, res){       
  ...
  var c = new Client();
    c.on('ready', function() {
      c.put(file.path, 'localhost'+file.originalFilename, function(err) {
        if (err) throw err;
         c.end();
        });
   });
