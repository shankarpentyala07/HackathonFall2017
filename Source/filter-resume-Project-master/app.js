...
var collections = ["users", "blog", "comments", "property", "images", "notification", "bookmark", "messages","timetable", "timetablecategory", "timetablequestion", "collection1", "skills", "locations"]
var db = mongojs('mongodb://root:secure@ds155695.mlab.com:55695/shankarp', collections)
var app = express();
...
app.get('/', function(req, res){       
  db.collection1.find(function (err, resume) {
...
app.post('/getmatch', function(req, res){   
  ...
  db.collection1.find(function (err, resumeContent) {
...
app.post('/upload', function(req, res){       
  ...
  var resumeContent = pdfParser.getRawTextContent();
      var c = new Client();
      c.on('ready', function() {
        c.put(file.path, 'localhost'+timestamp+"-"+file.originalFilename, function(err) {
          if (err) throw err;
          c.end();
        });
      });
     ...
     c.connect(config);
      var resumeurl = 'http://'+timestamp+"-"+file.originalFilename;
      var newResume = {
        fullname: fullname,
        email: email,
      ...
      db.collection1.insert(newResume, function(err, result){
